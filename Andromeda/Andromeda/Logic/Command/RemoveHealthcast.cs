        ...
        public override void RunCommand(string rawDeviceList)
        {
            ...
            foreach (var device in confirmedConnectionList)
            {
                ...
                if (remote != null)
                {
                    try
                    {
                        ...
                        using (var searcher = new ManagementObjectSearcher(remote, productquery0))
                        {
                            foreach (ManagementObject product in searcher.Get()) // this is the fixed line
                            {
                                ...
                                ResultConsole.AddConsoleLine("Called webplugin uninstall on device " + device + ".");
                        ...
                        using (var searcher = new ManagementObjectSearcher(remote, productquery1))
                        {
                            ...
                            {
                                product.InvokeMethod("uninstall", null);
                                ResultConsole.AddConsoleLine("Called proxcard uninstall on device " + device + ".");
                                Logger.Log("Called uninstall on device " + device + ".");
                            }
                        }
                        ...
                        using (var searcher = new ManagementObjectSearcher(remote, productquery2))
                        {
                            ...
                            {
                                product.InvokeMethod("uninstall", null);
                                ResultConsole.AddConsoleLine("Called exactaccess uninstall on device " + device + ".");
                                Logger.Log("Called uninstall on device " + device + ".");
                            }
                        }

                        // Remove the remaining data regarding healthcast
                        var allProgramsFolder = "\\\\" + device + "\\C$\\ProgramData\\Microsoft\\Windows\\Start Menu\\Programs\\HealthCast";
                        var programDataHealthcast = "\\\\" + device + "\\C$\\ProgramData\\HealthCast";
                        ...
                        if (Directory.Exists(allProgramsFolder))
                        {
                            try
                            {
                                ...
                                ResultConsole.AddConsoleLine("Removed Healthcast \"All Programs\" directory on " + device);
                            }
                            catch (Exception e)
                            {
                                Logger.Log("There was an exception deleting the Healthcast \"All Programs\" directory on " + device + "\n Exception: " + e.Message + " Inner exception: " + e.InnerException);
                                ResultConsole.AddConsoleLine("There was an exception trying to delete the Healthcast \"All Programs\" directory on " + device + ": " + e.Message);
                        ...
                        if (Directory.Exists(programDataHealthcast))
                        {
                            try
                            {
                                ...
                                ResultConsole.AddConsoleLine("Removed Healthcast \"ProgramData\" directory on " + device);
                            }
                            catch (Exception e)
                            {
                                Logger.Log("There was an exception deleting the Healthcast \"ProgramData\" directory on " + device + "\n Exception: " + e.Message + " Inner exception: " + e.InnerException);
                    ...
                    {
                        Logger.Log(ActionName + " threw exception " + ex.Message + " with inner exception: " + ex.InnerException);
                        ResultConsole.AddConsoleLine(ActionName + " threw exception " + ex.Message);
