...
namespace Andromeda_Actions_Core.Command
{
    public class CleanComputerCaches : Action
    {
        ...
        public CleanComputerCaches(INetworkServices networkServices, IFileAndFolderServices fileAndFolderServices) : base(networkServices, fileAndFolderServices)
        {
        ...
        public override void RunCommand(string rawDeviceList)
        {
            ...
            try
            {
                foreach (var device in devlist)
                {
                    ...
                    if (FileAndFolderServices.ValidateDirectoryExists(device, CcmCachePath, ActionName))
                    {
                        FileAndFolderServices.CleanDirectory(device, CcmCachePath);
                    }

                    if (FileAndFolderServices.ValidateDirectoryExists(device, WindowsTemp, ActionName))
                    {
                        FileAndFolderServices.CleanDirectory(device, WindowsTemp);
                    }
                    ...
                    {
                        if (FileAndFolderServices.ValidateDirectoryExists(device, userFolder, ActionName))
                        {
                            // Validate and Clean User Temp Folder at "C:\users\[user]\appdata\local\temp"
                            if (FileAndFolderServices.ValidateDirectoryExists(device, userFolder + UserTemp, ActionName))
                            {
                                FileAndFolderServices.CleanDirectory(device, userFolder + UserTemp);
                            }
                            ...
                            if (FileAndFolderServices.ValidateDirectoryExists(device, userFolder + UserTempInternetFiles, ActionName))
                            {
                                FileAndFolderServices.CleanDirectory(device, userFolder + UserTempInternetFiles);
                            }

                            // Validate and Clean User Java Cache Files at "C:\Users\[user]\AppData\Local\Microsoft\Windows\Temporary Internet Files"
                            if (FileAndFolderServices.ValidateDirectoryExists(device, userFolder + JavaCache, ActionName))
                            {
                                FileAndFolderServices.CleanDirectory(device, userFolder + JavaCache);
                            }
                        }
                    }
                }
